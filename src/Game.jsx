...
export default class Game extends React.Component {
  ...
  state = {
    playersTurn: true,
    player: [new Animal('Elefant', 'placeholder.png', 3.3, 6000, 70, 1, 40), new Animal('Flusspferd', 'placeholder.png', 1.5, 1800, 50, 1, 30)],
    computer: [new Animal('Nashorn', 'placeholder.png', 1.9, 2300, 50, 1, 50), new Animal('Krokodil', 'placeholder.png', '5.2', '1000', 70, 60, 29)],
    selectedProperty: '',
    computerUncovered: false,
  };

  getSelectPropertyHandler = property => {
    this.play(property);
  };

  play = property => {
    this.setState(
      state => update(state, { selectedProperty: { $set: property }, computerUncovered: { $set: true } }),
      () => {
        setTimeout(() => {
  ...
  compare = property => {
    ...
    this.setState(
      state => update(state, { $set: { computerUncovered: false, selectedProperty: '', playersTurn, player, computer } }),
      () => {
        if (!playersTurn) {
          setTimeout(() => {
  ...
  render() {
    ...
    return (
      <div>
        ...
        <div className="cards">
          <Card uncovered={true} animal={player[0]} selectedProperty={selectedProperty} onSelectProperty={this.getSelectPropertyHandler} />
          <Card uncovered={computerUncovered} animal={computer[0]} selectedProperty={selectedProperty} />
        </div>
      </div>
    );
  }
